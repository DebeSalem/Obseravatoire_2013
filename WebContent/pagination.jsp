...
<link rel="stylesheet" type="text/css" href="/Observatoire_2013/CSS/pagination.css" />
...
<body >
	...
	try{
		if((Integer.parseInt(request.getParameterValues("pageEncours")[0])) > 1) { 
			pageEncours  = Integer.parseInt(request.getParameterValues("pageEncours")[0]);
		}
		else{
			 pageEncours = 1;
		}
	...
	%>
	<% List<AncienEtudiant> etudiants = modele.tousLesEtudiants(1 + (4 * (pageEncours - 1)), 4); %>
	<b> Le nombre total = <%= etudiants.size() %></b>
	<div align = "center">
		<div id="container">
		<div id="header">
        	<h1>ISCAE</h1>
		...
		<% int nombreDepages = modele.tousLesEtudiants().size();
		nombreDepages /= 4;
		%>
		<b> [ Page : <% for(int i = 1; i <= nombreDepages; i++){
			if(i == pageEncours){
				out.print(i);
			}
			else{
		%>
				<form method = "POST" action = "http://localhost:8080/Observatoire_2013/pagination.jsp">
					<input type = "hidden" value = "<%= i%>" name = "pageEncours">
